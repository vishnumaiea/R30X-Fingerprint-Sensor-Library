...
#define RX_OK                        0x00U  //when the response is correct
#define RX_BADPACKET                 0x01U  //if the packet received from FPS is badly formatted
#define RX_WRONGRESPONSE             0x02U  //unexpected response
#define RX_TIMEOUT                   0x03U  //when no response was received
...
#define DEFAULT_TIMEOUT                 1000  //UART reading timeout in milliseconds
#define DEFAULT_BAUDRATE                57600 //9600*6
#define DEFAULT_RX_DATA_LENGTH          64    //the max length of data in a received packet
#define DEFAULT_SECURITY_LEVEL          3     //the threshold at which the fingerprints will be matched
#define DEFAULT_SERIAL_BUFFER_LENGTH    300   //length of the buffer used to read the serial data
#define DEFAULT_PASSWORD                0xFFFFFFFF
#define DEFAULT_ADDRESS                 0xFFFFFFFF
#define BAD_VALUE                       0x1FU //some bad value or paramter was delivered

#define FINGERPRINT_DEBUG   //uncomment this line to enable debug info to be printed
...
class R30X_Fingerprint {
  ...
  #if defined(__AVR__) || defined(ESP8266)
    R30X_Fingerprint (SoftwareSerial *ss, uint32_t password = DEFAULT_PASSWORD, uint32_t address = DEFAULT_ADDRESS);
  #endif
  
  R30X_Fingerprint (HardwareSerial *hs, uint32_t password = DEFAULT_PASSWORD, uint32_t address = DEFAULT_ADDRESS);
  ...
  uint8_t verifyPassword (uint32_t password = DEFAULT_PASSWORD); //verify the user supplied password
  uint8_t setPassword (uint32_t password);  //set FPS password
  uint8_t setAddress (uint32_t address = DEFAULT_ADDRESS);  //set FPS address
  ...
  uint8_t receivePacket (uint32_t timeout=DEFAULT_TIMEOUT); //receive packet from FPS
